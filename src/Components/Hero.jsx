...
const socialLinks = [
  { icon: <FaGithub size={20} />, url: "https://github.com/Riad-Zz", label: "GitHub" },
  { icon: <FaLinkedin size={20} />, url: "https://www.linkedin.com/in/riadul-islam-riad/", label: "LinkedIn" },
  { icon: <FaEnvelope size={20} />, url: "mailto:islamriad3242@gmail.com", label: "Email" },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 grid lg:grid-cols-[1.2fr_0.8fr] gap-10 lg:gap-16 items-center z-10">
        ...
        >
          <div className="relative group">
            <div className="w-[240px] h-[240px] sm:w-[320px] sm:h-[320px] md:w-[400px] md:h-[400px] lg:w-[460px] lg:h-[460px] relative rounded-full overflow-hidden border-4 md:border-[6px] border-white/20 bg-base-100/10 backdrop-blur-3xl shadow-2xl z-20">
              <img
                src={ProfileImg}
                alt="Md.Riadul Islam Riad"
        ...
        {/* Profile Image - Circle, Premium Style */}
        ...
        >
          <div className="relative group">
            {/* Main Circle Image */}
            <div className="w-[240px] h-[240px] sm:w-[320px] sm:h-[320px] md:w-[400px] md:h-[400px] lg:w-[460px] lg:h-[460px] relative rounded-full overflow-hidden border-4 md:border-6 border-white/20 bg-base-100/10 backdrop-blur-3xl shadow-2xl z-20">
              <img
                src={ProfileImg}
                alt="Md.Riadul Islam Riad"
                className="w-full h-full object-cover scale-100 group-hover:scale-110 transition-transform duration-1000 ease-out"
            ...
            {/* Outer Glow Layer - Primary Theme Color */}
            <div className="absolute inset-0 w-full h-full rounded-full border border-white/10 bg-primary/10 backdrop-blur-xl -z-10 scale-[1.05] md:scale-[1.1] group-hover:scale-[1.08] md:group-hover:scale-[1.12] transition-all duration-700" />

            {/* Subtle Rotating Glow with Theme Colors */}
            <div className="absolute -inset-2 rounded-full bg-gradient-to-r from-primary/40 via-accent/30 to-secondary/40 opacity-30 animate-spin-slow -z-20 pointer-events-none" />

            {/* Accent Border Halo */}
            <div className="absolute -inset-1 rounded-full border-2 border-accent/20 pointer-events-none -z-10" />
          </div>
        </motion.div>
        ...
        >
          ...
          <motion.h1 variants={textUpVariants} className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-tight lg:leading-none">
            MERN <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">DEVELOPER</span>
          </motion.h1>

          <motion.h2 variants={textUpVariants} className="text-lg sm:text-2xl md:text-3xl font-bold h-[30px] md:h-[40px] text-base-content/80">
          ...
          {/* Social Links */}
          <motion.div variants={textUpVariants} className="flex justify-center lg:justify-start gap-4 mt-2">
            {socialLinks.map((item, index) => (
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="p-3 rounded-full bg-primary/20 hover:bg-primary transition-colors duration-300 text-primary dark:text-white flex items-center justify-center text-xl shadow-md"
          ...
          <motion.div variants={textUpVariants} className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-6 md:pt-1">
            <motion.a
              href="#projects"
              whileHover={{ scale: 1.05, y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="relative group overflow-hidden px-8 md:px-12 py-3 md:py-5 rounded-full bg-gradient-to-br from-primary via-primary to-accent text-primary-content text-base md:text-lg font-black shadow-2xl text-center"
            >
